...
class sisSpider(CrawlSpider):
    ...
    def __init__(self, forum_id=58, digit=1, *args, **kwargs):
        ...
        self.rules = [
            Rule(sle(allow=("/forum/forum-" + str(forum_id) + "-[0-9]{," + str(digit) + "}\.html")), follow=True),
            Rule(sle(allow=("/forum/thread" + "-[0-9]{," + str(7) + "}")), callback = "get_img")
            ]
    ...
    def parse_2(self, response):
        ...
        for site in sites:
            item = SisItem()
            item['title'] = site.css('.postmessage h2::text').extract()[0]
    ...
    def get_img(self, response):
        ...
        item['images'] = sel.xpath('.//*[@class="postmessage defaultpost"]/h2/text()').extract()
        info('parsed ' + str(response))
        yield item
